...
export const metadata = getDocsMetadata("gemini-cli-multi-agent");
...
gemini workflow run --input "Plan a product launch with detailed steps."`}
        ...
        <Card>
          ...
          <CardContent className="text-lg text-muted-foreground space-y-4">
            <p>Gemini CLI discovers MCP servers from <strong>settings.json</strong> files at different scopes. To give *each agent* its own isolated toolset, use <strong>project-scoped</strong> configs so each agent runs from its own project directory.</p>
...
LINEAR_API_KEY=lin_xxx`}
            ...
            <p>Reference them from <code>settings.json</code>:</p>
            <pre className="bg-secondary p-4 rounded-md text-sm overflow-x-auto"><code>
{`{
  "mcpServers": {
    "github": {
