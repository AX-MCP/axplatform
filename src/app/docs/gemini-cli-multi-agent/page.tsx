      ...
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        <Card>
          ...
          <CardContent className="space-y-4 text-lg text-muted-foreground">
            ...
            <div>
              <h3 className="font-semibold text-foreground mb-2">Example configuration file (<code>agents.yaml</code>):</h3>
     ...
     - name: main-agent
       subagents:
        ...
        <Card>
            ...
            <CardContent className="text-lg text-muted-foreground">
                <p>You can orchestrate multiple agents using `gemini workflow` commands.</p>
...
gemini workflow run --input "Plan a product launch with detailed steps."`}
        ...
        <Card>
            ...
            <CardContent className="text-lg text-muted-foreground space-y-4">
                <p>Gemini CLI discovers MCP servers from <strong>settings.json</strong> files at different scopes. To give <em>each agent</em> its own isolated toolset, use <strong>project-scoped</strong> configs so each agent runs from its own project directory.</p>
                ...
                <ul className="list-disc list-inside space-y-2">
                  ...
                  <li><strong>Project scope:</strong> <code>./.gemini/settings.json</code> (applies only when you run `gemini` from this folder; <strong>recommended for per‑agent isolation</strong>)</li>
                </ul>
                <p className="italic text-sm">Tools defined at the project scope override/augment user scope. Keep sensitive API keys in env vars, not in JSON.</p>

                <h3 className="font-semibold text-xl text-foreground pt-4">7.2 Add MCP servers for <em>Agent A</em> (project A)</h3>
                ...
                <p>2) Add MCP servers to this project only:</p>
...
gemini mcp add --scope project --transport http linear https://mcp.linear.app/sse`}</code></pre>
                ...
                <h3 className="font-semibold text-xl text-foreground pt-4">7.4 Restrict tool visibility per agent (allowlist/denylist)</h3>
                 <p>Within each project’s <code>./.gemini/settings.json</code>, you can scope which tools from a server are exposed via `includeTools`/`excludeTools`.</p>
                <pre className="bg-secondary p-4 rounded-md text-sm overflow-x-auto"><code>
{`{
  "mcpServers": {
    "github": {
      ...
      "trust": false
                ...
                <p className="italic text-sm">Use `trust: false` for explicit confirmation on tool calls; set to `true` only for well‑audited servers.</p>


                <h3 className="font-semibold text-xl text-foreground pt-4">7.5 Per‑agent secrets</h3>
...
LINEAR_API_KEY=lin_xxx`}
                ...
                <p>Reference them from `settings.json`:</p>
                <pre className="bg-secondary p-4 rounded-md text-sm overflow-x-auto"><code>
{`{
  "mcpServers": {
    "github": {
      "headers": { "Authorization": "Bearer \${GITHUB_TOKEN}" }
                ...
                <h3 className="font-semibold text-xl text-foreground pt-4">7.7 Sub‑agents and MCP tools</h3>
